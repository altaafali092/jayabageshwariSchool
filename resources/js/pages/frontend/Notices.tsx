    ...
    return (
        <FrontLayout>
            ...
            <main className="flex-1 bg-white">
                ...
                {/* ================= SEARCH & SORT ================= */}
                <section className="sticky top-[80px] lg:top-[110px] z-40 border-b border-slate-100 py-6 px-6 lg:px-0 shadow-lg shadow-blue-900/5 backdrop-blur-md bg-slate-50/90">
                    <div className="container mx-auto lg:px-20 flex flex-col md:flex-row justify-between items-center gap-6">
                        <div className="flex items-center gap-6 w-full md:w-auto">
                            <div className="flex items-center gap-1.5">
                                ...
                                <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest italic">Active Updates</span>
                            </div>
                        ...
                        <div className="relative w-full md:w-96">
                            ...
                            <input
                                ...
                                className="w-full h-12 pl-12 pr-4 bg-white border border-slate-200 rounded-2xl text-[10px] font-black text-slate-900 outline-none focus:border-blue-600 focus:ring-4 focus:ring-blue-600/5 transition-all uppercase tracking-[0.2em] placeholder:text-slate-300"
                            />
                        </div>
                    </div>
                </section>
                        ...
                        <div className="space-y-6">
                            {filteredNotices.map((notice, i) => (
                                <div
                                    ...
                                    className={cn(
                                        ...
                                        notice.isPinned
                                            ? "bg-blue-50/30 border-blue-100 shadow-xl shadow-blue-950/5"
                                            : "bg-white border-slate-100 hover:bg-slate-50/50 hover:shadow-xl hover:shadow-slate-900/5"
                                    )}
                                >
                                    {notice.isPinned && (
                                    ...
                                    <div className="flex items-center gap-4 shrink-0">
                                        <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-2xl bg-white border border-slate-100 shadow-sm flex flex-col items-center justify-center shrink-0">
                                            <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">{notice.date.split(' ')[0]}</span>
                                            <span className="text-xl lg:text-2xl font-black text-slate-900 leading-none">{notice.date.split(' ')[1].replace(',', '')}</span>
                                        </div>
                                        <div className="md:hidden">
                                            <span className="px-3 py-1 rounded-lg bg-blue-100 text-blue-600 text-[9px] font-black uppercase tracking-widest">
                                                {notice.category}
                                            </span>
                                        </div>
                                    </div>
                                        ...
                                        <div className="hidden md:flex items-center gap-3">
                                            ...
                                            <span className={cn(
                                                ...
                                                notice.priority === 'High' ? "bg-red-100 text-red-600" :
                                                    notice.priority === 'Medium' ? "bg-orange-100 text-orange-600" : "bg-slate-100 text-slate-600"
                                            )}>
                                                {notice.priority} Priority
                                            </span>
                                        </div>
                                        <h3 className="text-xl lg:text-2xl font-black text-slate-900 uppercase italic tracking-tight leading-tight group-hover:text-blue-600 transition-colors">
                                            {notice.title}
                                        </h3>
                                    ...
                                    {/* Download Action */}
                                    <div className="w-full md:w-auto flex items-center gap-4 pt-4 md:pt-0 border-t md:border-t-0 border-slate-100">
                                        <button className="flex-1 md:flex-none h-14 bg-slate-900 text-white px-8 rounded-2xl flex items-center justify-center gap-3 text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all shadow-xl shadow-slate-900/10">
                                            <Download className="w-4 h-4" />
                                            <span>Download PDF</span>
                                        </button>
                                        <div className="hidden lg:block text-right">
                                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Filesize</p>
                                            <p className="text-xs font-black text-slate-900">{notice.fileSize}</p>
                                        </div>
                                    </div>
                                </div>
                            ))}
                            ...
                            {filteredNotices.length === 0 && (
                                <div className="py-20 text-center space-y-4">
                                    ...
                                    <p className="text-slate-400 font-extrabold uppercase tracking-widest text-sm">No notices found matching your search</p>
                                </div>
                            )}
                        </div>
                    </div>
                        ...
                        <div className="max-w-2xl mx-auto space-y-8">
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <h2 className="text-3xl lg:text-5xl font-black text-slate-900 uppercase italic tracking-tighter leading-none">
                                    NEVER MISS A <span className="text-blue-600">CRITICAL</span> UPDATE
                                </h2>
                                <p className="text-slate-500 font-bold text-xs lg:text-sm uppercase tracking-[0.2em] leading-relaxed">
                                    Get instant SMS or Email alerts for urgent announcements and emergency notices.
                                </p>
                            </div>
                            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                                <button className="w-full sm:w-auto px-10 h-16 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-slate-950 transition-all shadow-xl shadow-blue-600/20 italic">
                                    Subscribe to SMS
                                </button>
                                <button className="w-full sm:w-auto px-10 h-16 bg-white border-2 border-slate-900 rounded-2xl font-black text-xs uppercase tracking-widest text-slate-900 hover:bg-slate-900 hover:text-white transition-all shadow-xl shadow-slate-900/5 italic">
                                    Follow on Telegram
                                </button>
                            </div>
                        </div>
