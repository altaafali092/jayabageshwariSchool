                ...
                <section className="py-24 bg-white dark:bg-slate-950 transition-colors duration-300">
                    <div className="container mx-auto px-6 lg:px-20">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
                            <div className="relative aspect-square rounded-[3rem] overflow-hidden shadow-2xl shadow-blue-950/10 dark:shadow-black/50 group">
                                ...
                                <div className="absolute bottom-10 left-10 p-10 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-3xl shadow-xl border border-white/20 dark:border-slate-800">
                                    <h4 className="text-3xl font-black text-slate-900 dark:text-white italic tracking-tighter mb-2">30+</h4>
                                    <p className="text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-widest">Years of Impact</p>
                                </div>
                            </div>
                            <div className="space-y-8">
                                <div className="space-y-4">
                                    <h2 className="text-3xl lg:text-5xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter leading-tight">
                                        HOW IT ALL <span className="text-blue-600 dark:text-blue-500">BEGAN</span>
                                    </h2>
                                ...
                                <div className="grid grid-cols-2 gap-8 pt-6 border-t border-slate-100 dark:border-slate-800">
                                    <div className="space-y-2">
                                        <h5 className="text-lg font-black text-slate-900 dark:text-white uppercase italic">Our Mission</h5>
                                        <p className="text-xs text-slate-500 dark:text-slate-500 leading-relaxed">To empower students with knowledge and character to lead and serve.</p>
                                    </div>
                                    <div className="space-y-2">
                                        <h5 className="text-lg font-black text-slate-900 dark:text-white uppercase italic">Our Vision</h5>
                                        <p className="text-xs text-slate-500 dark:text-slate-500 leading-relaxed">To be a global benchmark for value-based excellence in education.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="container mx-auto px-6 lg:px-20">
                        <div className="text-center mb-20 space-y-4">
                            <h2 className="text-3xl lg:text-5xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">
                                OUR <span className="text-blue-600 dark:text-blue-500">JOURNEY</span> SO FAR
                            </h2>
                            <p className="text-slate-500 dark:text-slate-400 font-bold text-xs lg:text-sm uppercase tracking-widest">Key milestones in our educational heritage</p>
                        </div>

                        <div className="relative">
                            {/* Desktop Line */}
                            <div className="hidden lg:block absolute top-1/2 left-0 w-full h-px bg-slate-200 dark:bg-slate-800" />

                            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                                {milestones.map((ms, i) => (
                                    <div key={i} className="group relative pt-12 lg:pt-0 lg:pb-12 text-center space-y-6">
                                        <div className="hidden lg:block absolute top-[50%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-white dark:bg-slate-900 border-4 border-blue-600 group-hover:scale-150 transition-transform duration-300 z-10" />
                                        <div className="space-y-2">
                                            <span className="text-3xl lg:text-4xl font-black text-blue-600/10 dark:text-blue-400/10 italic group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-500">{ms.year}</span>
                                            <h4 className="text-lg font-black text-slate-900 dark:text-white uppercase italic tracking-tight">{ms.title}</h4>
                                        </div>
                                        <p className="text-xs font-bold text-slate-500 dark:text-slate-400 leading-relaxed uppercase tracking-tight px-4 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-500">
                                            {ms.desc}
                                        </p>
                                    </div>
                                ))}
                ...
                <section className="py-24 bg-slate-900 dark:bg-black text-center">
                    <div className="container mx-auto px-6 lg:px-20 space-y-12">
                        <div className="space-y-4">
                            ...
                            <p className="text-slate-400 dark:text-slate-500 font-bold text-xs lg:text-sm uppercase tracking-widest">Join the thousands of alumni who have shaped their future at JBS</p>
                        </div>
                        <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
                            <Link href="/admissions" className="w-full sm:w-auto h-20 px-12 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-white hover:text-slate-950 transition-all flex items-center justify-center gap-4 italic shadow-2xl shadow-blue-600/20 dark:shadow-blue-900/40 group">
                                Apply Now
