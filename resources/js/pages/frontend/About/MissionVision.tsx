...
const MissionVision = ({ categories, pageCategory }: PageProps) => {
    // Local tab state tracks internal sub-pages within the active selected category
    const [activeTab, setActiveTab] = useState<number>(0);
    const activePage = pageCategory?.pages?.[activeTab];
                    ...
                    <div className="container mx-auto px-6 lg:px-20">
                        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
                                        ...
                                        {categories.map((cat) => {
                                            ...
                                            return (
                                                <Link
                                                    key={cat.id}
                                                  href={pages(cat.slug)}
                                                    className={`text-left pl-6 py-4 border-l-2 transition-all duration-300 relative group flex items-center justify-between gap-2 w-full ${
                                                        isActive 
                            ...
                            <div className="lg:col-span-8 space-y-8">
                                ...
                                {pageCategory?.pages && pageCategory.pages.length > 1 && (
                                    <div className="flex flex-wrap gap-2 pb-4 border-b border-slate-100 dark:border-slate-900/60">
                                        {pageCategory.pages.map((subItem, idx) => (
                                            <button
                                                key={subItem.id}
                                                onClick={() => setActiveTab(idx)}
                                                className={`text-[11px] font-black uppercase tracking-wider px-4 py-2 rounded-xl transition-all border ${
                                                    activeTab === idx
                                ...
                                {activePage ? (
                                    <div className="space-y-8 animate-fadeIn">
                                        ...
                                        {/* Multi-Asset Gallery Matrix (Secondary Support Images) */}
