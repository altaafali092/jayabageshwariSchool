    ...
    return (
        <FrontLayout>
            ...
            <main className="flex-1 bg-white dark:bg-slate-950 transition-colors duration-300">
                ...
                {/* ================= TABS SECTION ================= */}
                <section className="sticky top-[80px] lg:top-[80px] z-40 bg-white dark:bg-slate-950 border-b border-slate-100 dark:border-slate-800 py-6 px-6 lg:px-0 transition-colors duration-300">
                    <div className="container mx-auto lg:px-20">
                        <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
                                ...
                                {tabs.map((tab) => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            activeTab === tab
                                                ? "bg-slate-900 dark:bg-blue-600 text-white shadow-xl shadow-slate-900/10 dark:shadow-blue-900/40"
                                                : "bg-slate-50 dark:bg-slate-900 text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 italic"
                                        )}
                                    >
                                        {tab}
                                    </button>
                            ...
                            <div className="flex items-center gap-3">
                                <span className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest italic">Grid Mode</span>
                                <div className="p-1.5 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl flex items-center gap-1">
                                    ...
                                    <div className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 dark:text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"><Play className="w-4 h-4" /></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="container mx-auto px-6 lg:px-20">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {filteredItems.map((item) => (
                                <div key={item.id} className="group relative aspect-square rounded-[2.5rem] overflow-hidden bg-slate-100 dark:bg-slate-900 hover:shadow-2xl hover:shadow-blue-950/10 dark:hover:shadow-black/50 transition-all duration-700">
                                    <img
                                        src={item.image}
                                        alt={item.title}
                                    ...
                                    <div className="absolute bottom-10 left-10 opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-500 delay-100">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-black text-blue-500 dark:text-blue-400 uppercase tracking-widest italic">{item.category}</p>
                        ...
                        {/* Pagination Meta */}
                        <div className="mt-20 pt-10 border-t border-slate-50 dark:border-slate-900 flex flex-col md:flex-row justify-between items-center gap-8">
                            <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em]">Showing {filteredItems.length} of {items.length} Campus Captures</p>
                            <button className="h-16 px-12 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-900 dark:text-white rounded-3xl font-black text-[10px] uppercase tracking-[0.3em] hover:bg-slate-900 dark:hover:bg-blue-600 hover:text-white transition-all italic flex items-center gap-3 shadow-sm">
                                Load More Moments
