            ...
            <div className="container relative z-10 px-6 mx-auto lg:px-12">
                <div className="flex flex-col lg:flex-row gap-16 items-center">
                    ...
                    <div className="lg:w-1/3 text-left space-y-6">
                        <div className="inline-flex items-center gap-2 px-3 py-1 text-xs font-black text-red-600 uppercase tracking-[0.2em] bg-red-50 border border-red-100 rounded-lg">
                            <ShieldAlert className="w-4 h-4" />
                            <span>Attention Required</span>
                        </div>
                        <h2 className="text-4xl font-extrabold tracking-tight text-slate-900 md:text-5xl">
                            Important <br />
                            <span className="text-red-600">Notice Bulletin</span>
                        </h2>
                        ...
                        <div className="flex gap-3 pt-4">
                            <button
                                onClick={prevNotice}
                                className="w-12 h-12 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-600 hover:bg-slate-900 hover:text-white transition-all shadow-sm active:scale-95"
                            >
                            ...
                            {notices.map((notice, index) => (
                                ...
                                >
                                    <div className="h-full bg-slate-50 rounded-[2.5rem] border border-slate-100 p-8 md:p-12 shadow-2xl shadow-slate-200/50 flex flex-col justify-between group overflow-hidden relative">
                                        ...
                                        <Megaphone className="absolute -bottom-10 -right-10 w-48 h-48 text-slate-200/20 -rotate-12 group-hover:text-red-600/5 transition-colors duration-500" />
                                            ...
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="flex flex-col">
                                                        <span className="text-xs font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">Posted On</span>
                                                        <span className="text-sm font-black text-slate-900 leading-none">{notice.date}</span>
                                                    </div>
                                                </div>
                                                <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${notice.urgency === 'High' ? 'bg-red-600 text-white shadow-lg shadow-red-200' : 'bg-slate-200 text-slate-600'
                                                    }`}>
                                                    {notice.urgency} Urgency
                                                </div>
                                            </div>

                                            <h3 className="text-2xl md:text-3xl font-black text-slate-900 group-hover:text-red-700 transition-colors duration-300">
                                                {notice.title}
                                            </h3>
                                        ...
                                        <div className="relative z-10 pt-8 border-t border-slate-200 flex items-center justify-between">
                                            <div className="flex items-center gap-2 text-slate-500 font-bold text-sm">
                                                <div className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                                                Category: {notice.type} Notice
                                            </div>
                                            <a href={`/notices/${notice.id}`} className="inline-flex items-center gap-2 text-red-600 font-black text-sm group/btn">
                                                Read Fully
                        ...
                        <div className="flex justify-center lg:justify-start gap-3 mt-12">
                            {notices.map((_, idx) => (
                                <button
                                    ...
                                    className={`relative h-1.5 rounded-full transition-all duration-500 overflow-hidden ${activeIndex === idx ? 'w-16 bg-red-600 shadow-lg shadow-red-100' : 'w-4 bg-slate-200 hover:bg-slate-300'
                                        }`}
                                >
                                    {activeIndex === idx && (
